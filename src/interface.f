...
! ================================================================ !
          ...
          subroutine C_to_F_string(c_str, f_str)
            ...
            character(len=200), intent(out) :: f_str

            f_str = ' '

            i = 1
            do while(c_str(i:i) /= c_null_char)
               i = i + 1
               if (i >= 200) exit
            end do

            if (i > 1) then
               f_str(1:i-1) = c_str(1:i-1)
               f_str(1:len_trim(f_str)) = f_str(1:len_trim(f_str))
            endif
